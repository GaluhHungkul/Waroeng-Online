    ...
    <div className="relative z-10 flex-1 overflow-auto">
        <ul className="grid grid-cols-2 gap-5  justify-items-center mx-auto mt-10 w-4/5 lg:grid-cols-4">
          {props.products.map((product: Products) => (
            <div key={product._id} className="my-5 relative lg:w-52 pb-10 lg:pb-10 flex flex-col items-center min-h-96 bg-gray-800">
              ...
              <button disabled={!product?.stock} onClick={() => addToCart(product)} className="absolute bottom-4 disabled:bg-gray-700 bg-gray-500 hover:bg-gray-600 active:bg-gray-700 w-4/5 rounded font-bold ">{product?.stock ? 'BUY' : 'SOLD OUT'}</button>
            </div>
          ))}
        </ul>        
    </div>
