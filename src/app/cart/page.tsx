            ...
            <div  className="flex flex-col lg:grid lg:grid-cols-2 relative lg:pb-32">
              {cart.map((product) => (
                <div key={product._id} className="flex gap-2 m-2 lg:gap-5">
                  {product.img ? <Image src={product.img} alt={product.name} className="size-24 lg:size-64"  width={150} height={150}/> : <div className="w-40 h-40 bg-white"></div>}
                  <div className="text-[12px] lg:text-xl relative flex flex-col lg:gap-4">
                    ...
                    <div className="flex gap-1 lg:absolute bottom-0">
                      <button onClick={() => deleteFromCart(product)} className="bg-gray-600 px-2 lg:px-5 lg:py-1 rounded  font-bold text-white hover:bg-gray-500 active:bg-gray-400">-</button>
                      <button onClick={() => addToCart(product)} className="bg-gray-600 px-2 lg:px-5 lg:py-1 rounded  font-bold text-white hover:bg-gray-500 active:bg-gray-400">+</button>
                    </div>
                  </div> 
                </div>
              ))}
              <div className="w-full  p-2 text-sm  border-t lg:absolute lg:bottom-0 lg:text-xl">
                ...
                <button disabled={loading} onClick={handleCheckout} className="bg-white text-black mt-2  w-full font-bold rounded cursor-pointer hover:bg-gray-200 active:bg-gray-300  disabled:bg-gray-300 lg:py-2">Checkout</button>
              </div>
            </div> 
            : 
